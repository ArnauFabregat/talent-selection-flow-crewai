...
from talentflow.talent_selection_flow.crews.cv_to_job_crew.crew import CVToJobCrew


def test_init_with_custom_parameters():
    """
    Test that CVToJobCrew initializes with custom parameters.
    """
    # Arrange
    # Act
    crew = CVToJobCrew(guardrail_max_retries=10, verbose=True)
...
def test_init_with_negative_retries():
    """
    Test that CVToJobCrew accepts negative guardrail_max_retries values.
    """
    # Arrange
    # Act
    crew = CVToJobCrew(guardrail_max_retries=-1)
...
def test_init_with_zero_retries():
    """
    Test that CVToJobCrew accepts zero guardrail_max_retries values.
    """
    # Arrange
    # Act
    crew = CVToJobCrew(guardrail_max_retries=0)
...
def test_identify_gaps_task_with_missing_tasks_config():
    ...
    # Arrange
    crew = CVToJobCrew()
    crew.agents_config = {"gap_identifier_agent": {"name": "Technical Auditor"}}
...
def test_generate_interview_questions_task_with_missing_tasks_config():
    ...
    # Arrange
    crew = CVToJobCrew()
    crew.agents_config = {"interview_question_generator_agent": {"name": "Interview Strategist"}}
    crew.tasks_config = {}  # Empty tasks_config
